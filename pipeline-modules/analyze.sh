...
if [ $skipdlc == false ]; then
	...
	if [ $skiplink == false ]; then
		python3 ${scripts}/link_old_model.py ${name} --cond ${cond} --behmodeldir ${behavior_model_dir} --wandmodeldir ${wand_model_dir}
	fi

	python ${pyvm}/dlc/run.py ${name} --step 5 --cond ${cond}
fi
