...
class MainFrame(BaseFrame):
    def __init__(self, parent, config, jump_unlabeled):
        ...
        vSplitter.SplitVertically(
            self.image_panel, self.choice_panel, sashPosition=int(self.gui_size[0] * 0.8)
        )
        vSplitter.SetSashGravity(1)
        self.widget_panel = WidgetPanel(topSplitter)
        topSplitter.SplitHorizontally(
            vSplitter, self.widget_panel, sashPosition=int(self.gui_size[1] * 0.83)
        )  # 0.9
    ...
    def saveDataSet(self, event):
        ...
        self.Dataframe.columns = self.Dataframe.columns.set_levels(
            [self.scorer.replace(self.scorer, self.humanscorer)], level=0)
